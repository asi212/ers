...
try:
    ...
    # Define paths #
    snxls_path = '//fileserver/produktion/Endtest/30_Seriennummern/Seriennummern_v2.xlsm'  ## path of seriennummern spreadsheet
    ...
    if yeschiller == True:  ### if chiller exists, then E2 is changed from integer to a dataframe
        ...
        data_dict1 = {  # Assign values to all fillable PDF fields here
            'sys': sys,
            'title': E2.at['1', 'st'],
            'tr': ' ',  ## INCOMPLETE
            't_sz': t_sz,
        ...
        if morethanchiller == True:
            write_fillable_pdf(template_path, write_path + lauf_num + '-2.pdf', data_dict1)
        else:
            write_fillable_pdf(template_path, write_path + lauf_num + '.pdf', data_dict1)
        write2 = True
        # opens file in browser
    ...
    ###############################################
    ## If Chuck is TA1XP300 then make it Kiste 3 ##
    ###############################################
    if yeschuck == True and yescontroller == False:
        if t_sz == '300mm' and D['Chuck'].__contains__('XP300'):
            temp_df = ref_chucks.loc['XP300']
            crates_ls[count-1] = temp_df.at['verpackung']
            gewicht_ls[count-1] = temp_df.at['system gewicht']
    ...
    if (
        ...
        write_fillable_pdf(template_path5, write_path5 + lauf_num + '.pdf', data_dict3)
        write4 = True
    ...
    if yeschuck == True:
        write_fillable_pdf(template_path3, write_path3 + lauf_num + '.pdf', data_dict4)

        write_fillable_pdf(template_path4, write_path4 + lauf_num + '.pdf', data_dict4)

        write_fillable_pdf(template_path6, write_path6 + lauf_num + '.pdf', data_dict4)

        write_fillable_pdf(template_path7, write_path7 + lauf_num + '.pdf', data_dict4)
        write5 = True
    ...
    ### NOTES ###
    #write1 = data_dict, warranty
    #write2 = data_dict1, warranty
    #write3 = data_dict2, versand
    #write4 = data_dict3, PT100
    #write5 = data_dict4, planarity, uniformaity, system check, chuck check

    #############################
    ####### Chiller Only ########
    #############################
    if write2 == True and write3 == True and write1 == False and write4 == False and write5 == False:
        input_pdf_path = '//fileserver/alle/ERSTools/Endtest Document Generator/chiller_only.pdf'
        output_pdf_path = newdir + "/Alle_Zusammen_ERS" + lauf_num + '.pdf'
        # Don't forget to change the datadictionary for each corresponding section ###
        template_pdf = pdfrw.PdfReader(input_pdf_path)
        i = 0
        while i <3: # for pages 1 - 3
            ...
            for annotation in annotations:
                if annotation[SUBTYPE_KEY] == WIDGET_SUBTYPE_KEY:
                    if annotation[ANNOT_FIELD_KEY]:
                        ...
                        if key in data_dict.keys():
                            annotation.update(
                                pdfrw.PdfDict(V='{}'.format(data_dict[key]))  # "data_dict" correspnds to Warranty sheet
                            )
            i = i + 1
        while i < 5: # for pages 4 - 5
            annotations = template_pdf.pages[i][ANNOT_KEY]
            for annotation in annotations:
                if annotation[SUBTYPE_KEY] == WIDGET_SUBTYPE_KEY:
                    if annotation[ANNOT_FIELD_KEY]:
                        ...
                        if key in data_dict2.keys():
                            annotation.update(
                                pdfrw.PdfDict(V='{}'.format(data_dict2[key]))  # 'data_dict2' corresponds to versand
                            )
            i = i + 1
        pdfrw.PdfWriter().write(output_pdf_path, template_pdf)
    ...
    ####### Chiller More ########
    #############################
    if write2 == True and write3 == True and write1 == True and write4 == False and write5 == False:
        input_pdf_path = '//fileserver/alle/ERSTools/Endtest Document Generator/chiller_more.pdf'
        output_pdf_path = newdir + "/Alle_Zusammen_ERS" + lauf_num + '.pdf'
        # Don't forget to change the datadictionary for each corresponding section ###
        template_pdf = pdfrw.PdfReader(input_pdf_path)
        i = 0
                ...
                if annotation[SUBTYPE_KEY] == WIDGET_SUBTYPE_KEY:
                    if annotation[ANNOT_FIELD_KEY]:
                        ...
                        if key in data_dict1.keys():
                            annotation.update(
                                pdfrw.PdfDict(V='{}'.format(data_dict1[key]))
                            )
            i = i + 1
        while i < 8: # for pages 7 - 8
            annotations = template_pdf.pages[i][ANNOT_KEY]
            for annotation in annotations:
                if annotation[SUBTYPE_KEY] == WIDGET_SUBTYPE_KEY:
                    if annotation[ANNOT_FIELD_KEY]:
                        ...
                        if key in data_dict2.keys():
                            annotation.update(
                                pdfrw.PdfDict(V='{}'.format(data_dict2[key]))
                            )
            i = i + 1
        pdfrw.PdfWriter().write(output_pdf_path, template_pdf)
    ...
    ####### TS010 Only ########
    #############################
    if write2 == True and write3 == True and write1 == False and write4 == True and write5 == False:
        input_pdf_path = '//fileserver/alle/ERSTools/Endtest Document Generator/ts010_only.pdf'
        output_pdf_path = newdir + "/Alle_Zusammen_ERS" + lauf_num + '.pdf'
        # Don't forget to change the datadictionary for each corresponding section ###
        template_pdf = pdfrw.PdfReader(input_pdf_path)
        i = 0
        ...
        while i < 5: # for pages 4 - 5
            ...
            i = i + 1
        while i < 6: # for page 5
            annotations = template_pdf.pages[i][ANNOT_KEY]
            for annotation in annotations:
                if annotation[SUBTYPE_KEY] == WIDGET_SUBTYPE_KEY:
                    if annotation[ANNOT_FIELD_KEY]:
                        key = annotation[ANNOT_FIELD_KEY][1:-1]
                        if key in data_dict3.keys():
                            annotation.update(
                                pdfrw.PdfDict(V='{}'.format(data_dict3[key]))  # 'data_dict3' corresponds to PT100
                            )
            i = i + 1
        pdfrw.PdfWriter().write(output_pdf_path, template_pdf)
    ...
    ####### Chiller Controller ########
    #############################
    if write2 == True and write3 == True and write1 == True and write4 == True and write5 == False:
        input_pdf_path = '//fileserver/alle/ERSTools/Endtest Document Generator/chiller_controller.pdf'
        output_pdf_path = newdir + "/Alle_Zusammen_ERS" + lauf_num + '.pdf'
        # Don't forget to change the datadictionary for each corresponding section ###
        template_pdf = pdfrw.PdfReader(input_pdf_path)
        i = 0
            ...
            for annotation in annotations:
                if annotation[SUBTYPE_KEY] == WIDGET_SUBTYPE_KEY:
                    if annotation[ANNOT_FIELD_KEY]:
                        ...
                        if key in data_dict3.keys():
                            annotation.update(
                                pdfrw.PdfDict(V='{}'.format(data_dict3[key]))
                            )
            i = i + 1
        pdfrw.PdfWriter().write(output_pdf_path, template_pdf)
    ...
    ####### System ########
    #############################
    if write2 == True and write3 == True and write4 == True and write5 == True:
        input_pdf_path = '//fileserver/alle/ERSTools/Endtest Document Generator/system.pdf'
        output_pdf_path = newdir + "/Alle_Zusammen_ERS" + lauf_num + '.pdf'
        # Don't forget to change the datadictionary for each corresponding section ###
        template_pdf = pdfrw.PdfReader(input_pdf_path)
        i = 0
            ...
            for annotation in annotations:
                if annotation[SUBTYPE_KEY] == WIDGET_SUBTYPE_KEY:
                    if annotation[ANNOT_FIELD_KEY]:
                        ...
                        if key in data_dict3.keys():
                            annotation.update(
                                pdfrw.PdfDict(V='{}'.format(data_dict3[key])) #PT100
                            )
            i = i + 1
        while i < 13:  # for pages 10 - 13
            annotations = template_pdf.pages[i][ANNOT_KEY]
            for annotation in annotations:
                if annotation[SUBTYPE_KEY] == WIDGET_SUBTYPE_KEY:
                    if annotation[ANNOT_FIELD_KEY]:
                        key = annotation[ANNOT_FIELD_KEY][1:-1]
                        if key in data_dict4.keys():
                            annotation.update(
                                pdfrw.PdfDict(V='{}'.format(data_dict4[key]))  # PT100
                            )
            i = i + 1
        pdfrw.PdfWriter().write(output_pdf_path, template_pdf)
