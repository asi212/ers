...
try:
    ...
    # Import XLS #
    D = pandas.read_excel(snxls_path, sheet_name = 'Serien_Nummern aufsteigend', index_col = 0) # imports Seriennummern spreadsheet
...
# cancel script if lauf number is listed more than once in the excel file
    ...
    if lauf_num[0] != 'L':
        newdir = "//fileserver/alle/ERSTools/Endtest Document Generator/" + last4 + '_ERS' + lauf_num
    else:
        newdir = "//fileserver/alle/ERSTools/Endtest Document Generator/" + last4 + '_' + lauf_num
    ...
    if D['Chuck'] == D['Chuck']:
        ...
        if t_sz[0:4] == 'TC25':
            ...
            t_sz = t_sz.lstrip('aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPqQrRsStTuUvVwWxXyYzZ')  ## remove all letters from beginning
            removed_len = start_len - len(t_sz)
            t_sz = list(t_sz)  # convert string to list

            if len(t_sz) != 1:
                i = 0
                while t_sz[i].isdigit() and i < len(t_sz):      ## find position of first non-numeric character
                    i = i + 1
            else:
                i = 1

            t_sz = t_sz[0:i]   ##   get rid of all characters after the number ends
            ...
            if t_sz == '300':
                ...
                t_sz = ''
                print('Achtung!: Chuck größe konnte nicht bestimmt werden. Daher konnten Boxentyp und -gewicht ebenfalls nicht ermittelt werden. Bitte füllen Sie mit einem Stift aus.')
    ...
    if D['Chuck'] == D['Chuck']:
        if D['Chuck'][removed_len - 1] == 'P':
            prime = True
    if D['Chiller'] == D['Chiller']:
        if ('20P' in D['Chiller']) or ('08P' in D['Chiller']) or ('16P'in D['Chiller']) or ('10P' in D['Chiller']) : # if the chiller is a Prime series
            prime = True
    ...
    data_dict = {          ### Assign values to all fillable PDF fields here
        ...
        't_sz' : t_sz,
        'or_0' : '# ' + lauf_num,
        'or_1': E.at['1', 'or'],     ## in ['1', 'or'], '1' is a string instead of integer because we conocated E earlier to return length = 5
        'or_2': E.at['2', 'or'],
        'or_3': E.at['3', 'or'],
        'or_4': E.at['4', 'or'],
        'or_5': E.at['5', 'or'],
    ...
    # Import XLS #
    ref = pandas.read_excel(ref_path, dtype = str) # imports Seriennummern spreadsheet
    ...
    if yescontroller == True:
        ...
        if ref_controllers.at[controller_index, 'st'] == 'VG5':
            yesvg5 = True
            print('Achtung!:  Gewicht und Kiste #2 bleiben leer, da der Versand der VG5XX-Controller nicht bekannt war, als dieses Programm erstellt wurde. Sie müssen dies mit einem Stift ausfüllen.')
    ...
    if yeschuck == False and yeschiller == False and ((yescontroller == True and yesrsi == True) or yescontroller == False): # if only an option (no chuck, controller, or chiller), leave blank
        gw = ''
        i = 0
    ...
    if (yescontroller == True and yesrsi == False) or yests010 == True:   # if its a controller or a TS010 chiller, then make the file
