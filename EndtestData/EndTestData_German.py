...
global hideForChuckDept # Hide PT100 calibration and temperature uniformity
global hideForFTDept    # Hide Planarity
global hideCreateDatalogs #Hide CreateNew and EditList page (which deal with adding and removing datalogs)
global hideDocumentProblems #Hide the "Document Problems" Page. Note: This page is a duplicate of the program DocumentProblems listed at Alle/ERSTools/DocumentProblem
global dontUseActiveList    # Prevents the Problem Documentor from requireing that an LF is in the active list and/or seriennummern xls. This way, you can make a complaint about any LF, even if it isn't in the seriernnummern xls yet
hideForChuckDept = False
hideForFTDept = False
hideCreateDatalogs = False
hideDocumentProblems = False
dontUseActiveList = True
...
try:
    class SampleApp(tk.Tk):
        def __init__(self, *args, **kwargs):
            ...
            if hideDocumentProblems == False:
                ...
                for F in (StartPage, CreateNew, EditList, ModLF, Manager, TempLow, TempMed, TempHigh, TempOthr1, TempOthr2,
                          TempOthr3, PlanLow, PlanMed, PlanHigh, PlanOthr1, PlanOthr2, PlanOthr3, Pt100):
                    page_name = F.__name__
                    frame = F(parent=container, controller=self)
                    self.frames[page_name] = frame
        ...
        def update_manager(self, lf):
            ...
            self.noHighTemp = True
            if str(LF_status[4].tolist())[2:-2] != '999':
                self.noHighTemp = False
                if hideForChuckDept == False:
    ...
    class StartPage(tk.Frame):
        ...
        def __init__(self, parent, controller):
            ...
            continue_button = tk.Button(self, text="Fortfahren", command=self.continue_func) # command=self.continue_func will wait for button press to call function,
            ...
            newLF_button = tk.Button(self, text="Neuen Datenlog erstellen", command=lambda: controller.show_frame("CreateNew")) # if a function must be called with "()" at the end,
            ...
            if hideCreateDatalogs == True:
                newLF_button.grid_remove()


            editList_button = tk.Button(self, text="Datenlog Aktivieren/Deakt.", command=lambda: controller.show_frame("EditList"))
            ...
            if hideDocumentProblems == False:
                docDev_button = tk.Button(self, text="Problem Melden",
        ...
        def continue_func(self):
            if self.choice_var.get() != "Select Lauf Nummer":
                ...
                messagebox.showinfo("Error", "Bitte wählen Sie eine Laufnummer")
    ...
    class CreateNew(tk.Frame):
        ...
        def __init__(self, parent, controller):
            tk.Frame.__init__(self, parent)
            self.controller = controller
            self.label = tk.Label(self, text="Prüftemperatur 3 bei heißem System leer lassen")
            ...
            newLF_label = tk.Label(self, text="Neu LF")
            ...
            tempLow_label = tk.Label(self, text="Prüftemperatur 1")
            ...
            tempMed_label = tk.Label(self, text="Prüftemperatur 2")
            ...
            tempHigh_label = tk.Label(self, text="Prüftemperatur 3")
            ...
            self.tempOthr1_label = tk.Label(self, text="Prüftemperatur Andere1")
            ...
            self.tempOthr2_label = tk.Label(self, text="Prüftemperatur Andere2")
            ...
            self.tempOthr3_label = tk.Label(self, text="Prüftemperatur Andere3")
            ...
            HTU_button = tk.Button(self, text="Weitere Temperaturen hinzufügen für HTU", command=self.show_HTU)
            HTU_button.grid(row=9,column=1, columnspan=7, padx=5, pady=10)
            HTU_button.configure(width=33)

            return_button = tk.Button(self, text="Zurück", command=lambda: controller.show_frame("StartPage"))
            return_button.grid(row=10, column=2,columnspan=1, padx=10, pady=10, sticky = "E")
            return_button.configure(width=12)

            self.continue_button = tk.Button(self, text="Fortfahren", command=lambda: self.specs(list, sixpoints))
            self.continue_button.grid(row=10,column=3, columnspan=2, padx=10, pady=10)
            self.continue_button.configure(width=12)

            self.saveLF_button = tk.Button(self, text="Speichern", command=lambda: self.save_LF(self.type, self.yes_pt100,
        ...
        def specs(self, list, sixpoints):
            ...
            if sixpoints == True:
                if othr1_temp == "" or othr2_temp == "" or othr3_temp == "": # check to make sure all 'other' temperatures are given if the HTU button is pressed
                    messagebox.showinfo("fehlende Werte", "Bitte Resttemperaturen ausfüllen oder Programm neu starten")
        ...
        def save_LF(self, type, yes_pt100, last4, sixpoints):
            ...
            if self.check_var.get() == 0:
                ...
                if self.only2temps == False:
                    if self.choice_var_high.get() == 'deg C':
                        self.high_spec = str(self.specHigh_textbox.get('1.0', tk.END)).strip().replace(",",".")
                    else:
                        self.high_spec = str(0.01*float(str(self.tempHigh_textbox.get('1.0', tk.END)).strip().replace(",",".")) * \
                                        float(str(self.specHigh_textbox.get('1.0', tk.END)).strip().replace(",",".")))
                else:
                    self.high_spec = ''
            ...
            if self.check_var.get() == 0:
                ...
                new.loc[5] = str(self.specLow_textbox.get('1.0', tk.END)).strip().replace(",",".")
                new.loc[6] = str(self.specMed_textbox.get('1.0', tk.END)).strip().replace(",",".")
                ...
                if sixpoints == True:
                    ...
                    new.loc[26] = str(self.specOthr1_textbox.get('1.0', tk.END)).strip().replace(",",".")
                    new.loc[27] = str(self.specOthr2_textbox.get('1.0', tk.END)).strip().replace(",",".")
                    new.loc[28] = str(self.specOthr3_textbox.get('1.0', tk.END)).strip().replace(",",".")
            ...
            if len(LF_list[LF_list[0].astype(str).str.contains(new.loc[0]) == True]):
                messagebox.showinfo("Error", "Eintrag für Laufnummer bereits vorhanden")
                exists = True
...
##############
            ...
            i = 0
            x = 3
            if self.only2temps == True:
                x = 2
            while i < x:                            # THIS IS IMPORTANT
            ...
            if save == True:
                ...
                messagebox.showerror("Error",
                                     "Planaritätsspezifikation nicht akzeptabel (akzeptierte Werte = 8, 10, 12 und 15")
            else:
                messagebox.showinfo("Error", "fehlende Werte")

            if saved == True:
                messagebox.showinfo("New Message", "Speichern war erfolgreich")
        ...
        def get_type(self, new_LF):
            ...
            if alpha == True:
                if lauf_num not in D.index.values:
                    messagebox.showerror("Error", "Laufnummer nicht in Seriennummern-Tabelle")
                    x = 1/0
                else:
                    D = D.loc[str(lauf_num)]
            if alpha == False:
    ...
    class EditList(tk.Frame):
        def __init__(self, parent, controller):
            ...
            self.toInactive_button = tk.Button(self, text = "Inaktivieren ====>", command=lambda: self.toInactive())
            self.toInactive_button.grid(row = 4, column = 2, pady = 5)

            self.toActive_button = tk.Button(self, text = "<==== Aktivieren", command=lambda: self.toActive())
            self.toActive_button.grid(row = 5, column = 2, pady = 5)

            self.toActive_button = tk.Button(self, text = "Löschen", command=lambda: self.toEdit())
            self.toActive_button.grid(row = 6, column = 2, pady = 5)


            return_button = tk.Button(self, text="Zurück",
        ...
        def toInactive(self):
            ...
            if string != "":
                if len(active_list) > 1:
                    ...
                    messagebox.showinfo("Error", "Sie müssen immer mindestens einen LF in der aktiven Liste belassen.")
    ...
    class ModLF(tk.Frame):
        def __init__(self, parent, controller):
            ...
            return_button = tk.Button(self, text="Zurück", command=lambda: controller.show_frame("EditList"))
            return_button.grid(row=9, column=2, columnspan=1, padx=5, pady=10)
            return_button.configure(width=12)

            self.save_button = tk.Button(self, text="Speichern", command=lambda: self.mod_Save())
            self.save_button.grid(row=9, column=3, columnspan=1, padx=5, pady=10)
            self.save_button.configure(width=12)
            self.save_button.grid_remove()

            self.delete_button = tk.Button(self, text="Löschen", command=lambda: self.mod_Delete())
        ...
        def mod_Save(self):
            ...
            writer.save()
            self.controller.get_page("StartPage").update_LF_menu()
            messagebox.showinfo("Message", "Speichern war erfolgreich")

        def mod_Delete(self):
            delete = False
            MsgBox = messagebox.askquestion('Delete Record', 'Möchten Sie diesen Datensatz wirklich löschen?',
                                               icon='warning')
            if MsgBox == 'yes':
                delete = True
            ...
            if delete == True:
                ...
                messagebox.showinfo("Message", "Datensatz gelöscht)
    ...
    if hideDocumentProblems == False:
        class DocDev(tk.Frame):
            def __init__(self, parent, controller):
                tk.Frame.__init__(self, parent)
                self.controller = controller
                self.label = tk.Label(self, text="Anleitung: LF-Nummer auswählen oder eingeben. Klicken Sie auf Weiter. Füllen Sie alle Felder aus und speichern Sie dann.")
                ...
                return_button = tk.Button(self, text="Zurück",
                                          command=lambda: controller.show_frame("StartPage"))
                return_button.grid(row=10, column=1, padx=5, pady=10, sticky="E")
                return_button.configure(width=10)

                self.next_button = tk.Button(self, text="Next", command=lambda: next())
                self.next_button.grid(row=10, column=2, padx=5, pady=10)
                self.next_button.configure(width=10)

                self.save_button = tk.Button(self, text="Speichern", command=lambda: save(lauf_num))
                self.save_button.grid(row=10, column=3, padx=5, pady=10)
                self.save_button.configure(width=10)
                self.save_button.grid_remove()

                self.reset_button = tk.Button(self, text="Rücksetzen", command=lambda: reset())
                ...
                def next():
                    ...
                    if dontUseActiveList == False:
                        ...
                        elif len(self.enter_other_entry.get().strip()) > 3:
                            lauf_num = self.enter_other_entry.get().strip()
                        elif self.active_var.get() == "Select LF" and len(self.enter_other_entry.get().strip()) < 4:
                            messagebox.showerror("Error", "Bitte wählen Sie eine gültige LF aus oder geben Sie sie ein")
                    else:
                        if len(self.enter_other_entry.get().strip()) > 3:
                            lauf_num = self.enter_other_entry.get().strip()
                        else:
                            messagebox.showerror("Error", "Bitte geben Sie eine gültige LF ein ")
                ...
                def save():
                    ...
                    if dontUseActiveList == False:
                        if self.active_var.get() != "Select LF" and self.enter_other_entry.get().strip() == "":
                            ...
                            messagebox.showerror("Error", "Bitte wählen Sie einen gültigen LF aus oder geben Sie ihn ein.")
                    else:
                        if len(self.enter_other_entry.get().strip()) > 3:
                            lauf_num2 = self.enter_other_entry.get().strip()
                        else:
                    ...
                    if lauf_num2 != lauf_num:
                        messagebox.showerror("Error", "Die LF-Nummer wurde geändert. Bitte drucken Sie 'Reset' und klicken Sie auf 'Next' ")
                        save = False
                    ...
                    if save == True:
                        ...
                        try:
                            writer.save()
                            messagebox.showinfo("Success", "Speichern war erfolgreich")
                        except:
                            pass
                            messagebox.showerror("Error",
                                                 "Speichern war nicht erfolgreich.")  # if you get this message, its probably because the error_list.xlsx is open
    ...
    class Manager(tk.Frame):
        ...
        def __init__(self, parent, controller):
            ...
            ###
            self.temp4 = tk.Button(self, text="Temp Uniformity Andere1", font=controller.label_font, bg="white",
            ...
            self.temp5 = tk.Button(self, text="Temp Uniformity Andere2", font=controller.label_font, bg="white",
            ...
            self.temp6 = tk.Button(self, text="Temp Uniformity Andere3", font=controller.label_font, bg="white",
            ...
            self.plan4 = tk.Button(self, text="Planarity Andere1 ", font=controller.label_font, bg="white",
            ...
            self.plan5 = tk.Button(self, text="Planarity Andere2 ", font=controller.label_font, bg="white",
            ...
            self.plan6 = tk.Button(self, text="Planarity Andere3", font=controller.label_font, bg="white",
            ...
            if hideForChuckDept == False:
                button = tk.Button(self, text="Dateien generieren",
        ...
        def gen_Files(self):
            ...
            try:
                ...
                messagebox.showwarning("Warning", "Pruftemperatur1-Gleichmäßigkeitsdaten wurden nicht eingegeben")
...
# repeat all of this for the other temperatures
            ...
            try:
                ...
                messagebox.showwarning("Warning", "Pruftemperatur2-Gleichmäßigkeitsdaten wurden nicht eingegeben")
            ...
            try:
                ...
                if self.controller.noHighTemp == False:
                    messagebox.showwarning("Warning", "Pruftemperatur3-Gleichmäßigkeitsdaten wurden nicht eingegeben")
            ...
            try:
                ...
                messagebox.showwarning("Warning", "Niedertemperatur1-Planaritätsdaten wurden nicht eingegeben ")
            ...
            try:
                ...
                messagebox.showwarning("Warning", "Niedertemperatur2-Planaritätsdaten wurden nicht eingegeben ")
            ...
            try:
                ...
                if self.controller.noHighTemp == False:
                    messagebox.showwarning("Warning", "Niedertemperatur3-Planaritätsdaten wurden nicht eingegeben ")
            ...
            if data_ref.loc[4, 0] == '999':
                high_temp_val = ''
                checked3 = ''
            else:
                high_temp_val = data_ref.loc[4, 0]
                checked3 = 'X'

            if str(data_ref.loc[18, 0]) == 'nan':
                data_ref.loc[18, 0] = ''
            ...
            data_dict4 = {
                ...
                'med_temp1': data_ref.loc[3, 0],
                'med_temp2': data_ref.loc[3, 0],
                'high_temp1': high_temp_val,
                'high_temp2': high_temp_val,
           ...
           # if data_ref.loc[4, 0] == '999':
            #    phigh_spec_val = ''
          #  else:
            #phigh_spec_val = '± ' + phigh
           # print('=' + phigh_spec_val)
            data_dict5 = {
                'sys': sys,
                'chk': E.at[str(chuck_pos), 'st'],
                'chk_sn': E.at[str(chuck_pos), 'sn'],
                ...
                'pmed_temp1': data_ref.loc[3, 0],
                'pmed_temp2': data_ref.loc[3,0],
                'phigh_temp1': high_temp_val,
                'phigh_temp2': high_temp_val,
            ...
            Pt100_missing = False
            try:
                data_Pt100_min = float(data_Pt100.loc[0, 0].tolist()[0].replace(",", "."))
            except:
                messagebox.showwarning("Warning", "Pt100-daten velleicht wurden nicht eingegeben ")
                Pt100_missing = True
            ...
            template_path3 = '//fileserver/Alle/ERSTools/EndtestData/planarity_digital.pdf'
            write_path3_1 = dir1 + "/Planarity_digitial_ERS"
            write_path3_2 = dir2 + "/Planarity_digitial_ERS"
            template_path4 = '//fileserver/Alle/ERSTools/EndtestData/temperature_digital.pdf'
            write_path4_1 = dir1 + "/Temperature_digitial_ERS"
            write_path4_2 = dir2 + "/Temperature_digitial_ERS"
            template_path5 = '//fileserver/Alle/ERSTools/EndtestData/Pt100_calibration_digital.pdf'
            ...
            if yeschuck == True:
                ...
                if gen_plan == False:
                    messagebox.showinfo("Error", "Planaritätsprotokoll nicht generiert. Überprüfen Sie, ob die Datei bereits geöffnet ist oder ob der Ordner vorhanden ist")
                if gen_temp == False:
                    messagebox.showinfo("Error", "Temperaturgleichmäßigkeitsprotokoll nicht erstellt. Überprüfen Sie, ob die Datei bereits geöffnet ist oder ob der Ordner vorhanden ist")
            ...
            if (yescontroller == True and yesrsi == False and yesvg5 == False) or yests010 == True:
                ...
                try:
                    ...
                    pass
                if gen_pt100 == False:
                    messagebox.showinfo("Error", "Pt100-Protokoll nicht generiert. Überprüfen Sie, ob die Datei bereits geöffnet ist oder ob der Ordner vorhanden ist")
            ...
            if data_ref.loc[23,0] != '999' or data_ref.loc[24,0] != '999' or data_ref.loc[25,0] != '999': # if second set of temperatures exist
                ...
                try:
                    ...
                    messagebox.showwarning("Warning", "Andere1 Temperature Uniformity-Daten wurden nicht eingegeben")
                ...
                try:
                    ...
                    messagebox.showwarning("Warning", "Andere2 Temperature Uniformity-Daten wurden nicht eingegeben")
                ...
                try:
                    ...
                    messagebox.showwarning("Warning", "Andere3 Temperature Uniformity-Daten wurden nicht eingegeben")
                    Tempothr1_missing = True
                    pass
                ...
                try:
                    ...
                    messagebox.showwarning("Warning", "Andere1 Temperature Planarity-Daten wurden nicht eingegeben")
              ...
              #  else:
               #     phigh = str(data_ref.loc[7,0]) + "  deg C"
                ...
                template_path3 = '//fileserver/Alle/ERSTools/EndtestData/planarity_digital_2.pdf'
                write_path3_1 = dir1 + "/Planarity_digitial_ERS"
                write_path3_2 = dir2 + "/Planarity_digitial_ERS"
                template_path4 = '//fileserver/Alle/ERSTools/EndtestData/temperature_digital_2.pdf'
                write_path4_1 = dir1 + "/Temperature_digitial_ERS"
                write_path4_2 = dir2 + "/Temperature_digitial_ERS"
                ...
                if yeschuck == True:
                    ...
                    try:
                        ...
                        pass
                    if gen_plan2 == False:
                        messagebox.showinfo("Error", "Zweites Planaritätsprotokoll nicht generiert. Überprüfen Sie, ob die Datei bereits geöffnet ist oder ob der Ordner vorhanden ist.")
                    if gen_temp2 == False:
                        messagebox.showinfo("Error", "Zweites Temperaturgleichförmigkeitsprotokoll nicht erstellt. Bitte überprüfen Sie, ob die Datei bereits geöffnet ist oder ob der Ordner existiert")
    ...
    class TempLow(tk.Frame):
        ...
        def __init__(self, parent, controller):
            ...
            save = tk.Button(self, text="Speichern",
                               command=self.save)
            save.grid(row=14, column=2, padx=30, pady=20)

            self.checkComplete_var = tk.IntVar()
            self.checkComplete = tk.Checkbutton(self, text="Fertig", variable=self.checkComplete_var)
            self.checkComplete.grid(row=14, column=4, padx=0, pady=5)
            self.checkIgnore_var = tk.IntVar()
            self.checkIgnore = tk.Checkbutton(self, text="Fehler ignorieren (trotzdem speichern)", variable=self.checkIgnore_var)
        ...
        def save(self):
            ...
            if save == True:
                ...
                messagebox.showinfo("Error", "Speichern nicht erfolgreich")
